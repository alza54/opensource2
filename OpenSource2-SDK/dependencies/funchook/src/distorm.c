...
_DLLEXPORT_ _DecodeResult distorm_decompose64(
    _CodeInfo* ci, _DInst result[], unsigned int maxInstructions,
...
#endif
{
  ...
  if ((ci == NULL) || (ci->codeLen < 0) ||
      ((unsigned)ci->dt > (unsigned)Decode64Bits) || (ci->code == NULL) ||
      (result == NULL) || (maxInstructions == 0) ||
      ((ci->features & (DF_MAXIMUM_ADDR16 | DF_MAXIMUM_ADDR32)) ==
       (DF_MAXIMUM_ADDR16 | DF_MAXIMUM_ADDR32))) {
    return DECRES_INPUTERR;
  }

  return decode_internal(ci, FALSE, result, maxInstructions,
                         usedInstructionsCount);
}

#ifndef DISTORM_LIGHT

/* Helper function to concatenate an explicit size when it's unknown from the
 * operands. */
static void distorm_format_size(unsigned char** str, const _DInst* di,
                                int opNum) {
  int isSizingRequired = 0;
  /*
   * We only have to output the size explicitly if it's not clear from the
   * operands. For example: mov al, [0x1234] -> The size is 8, we know it from
   * the AL register operand. mov [0x1234], 0x11 -> Now we don't know the size.
   * Pam pam pam
   *
   * If given operand number is higher than 2, then output the size anyways.
   */
  isSizingRequired =
      ((opNum >= 2) || ((opNum == 0) && (di->ops[0].type != O_REG) &&
                        (di->ops[1].type != O_REG)));

  /* Still not sure? Try some special instructions. */
  if (!isSizingRequired) {
    /*
     * INS/OUTS are exception, because DX is a port specifier and not a real
     * src/dst register. A few exceptions that always requires sizing: MOVZX,
     * MOVSX, MOVSXD. ROL, ROR, RCL, RCR, SHL, SHR, SAL, SAR. SHLD, SHRD.
...
static void distorm_format_signed_disp(unsigned char** str, const _DInst* di,
                                       uint64_t addrMask) {
  int64_t tmpDisp64;
...
static uint8_t prefixTable[6][8] = {"",       "LOCK ", "REPNZ ",
                                    "REPNZ ", "REP ",  "REPZ "};
static unsigned int prefixSizesTable[6] = {0, 5, 6, 6, 4, 5};
static uint8_t suffixTable[10] = {0, 'B', 'W', 0, 'D', 0, 0, 0, 'Q'};

/* WARNING: This function is written carefully to be able to work with same
 * input and output buffer in-place! */
#ifdef SUPPORT_64BIT_OFFSET
_DLLEXPORT_ void distorm_format64(const _CodeInfo* ci, const _DInst* di,
                                  _DecodedInst* result)
#else
_DLLEXPORT_ void distorm_format32(const _CodeInfo* ci, const _DInst* di,
                                  _DecodedInst* result)
#endif
{
  unsigned char* str;
  ...
  if (ci->features & DF_USE_ADDR_MASK)
  ...
  /* Gotta have full address for (di->addr - ci->codeOffset) to work in all
   * modes. */
  str_hex(&result->instructionHex,
          (const uint8_t*)&ci->code[(unsigned int)(di->addr - ci->codeOffset)],
          di->size);
  ...
  if ((int)((int16_t)di->flags) == -1) {
    ...
    *(uint64_t*)&result->operands =
        0;  /* Clears length and the string at once. */
  ...
  if ((di->opcode >= I_MOVS) && (di->opcode <= I_SCAS)) {
    ...
    if ((SEGMENT_IS_DEFAULT_OR_NONE(di->segment)) &&
        (FLAG_GET_ADDRSIZE(di->flags) == ci->dt)) {
  ...
  for (i = 0; i < di->opsNo; i++) {
    ...
    if (type == O_REG) {
      ...
      if ((di->flags & FLAG_IMM_SIGNED) && (di->ops[i].size == 8) &&
        ...
        if (di->ops[i].size == 0x20)
          str_int(&str, di->imm.dword);
        else
...
#else
      tmpDisp64 = ((_OffsetType)di->imm.sdword + di->addr + di->size) &
                  (uint32_t)addrMask;
      str_int(&str, tmpDisp64);
#endif
    } else if (type == O_DISP) {
       ...
       * The problem with CMPS and MOVS is that they have two(!) memory
       * operands. So we have to complement(!) them ourselves, since the
       * isntruction structure supplies only the segment that can be overridden.
       ...
       */
      if (suffixSize == 0) {
        if (((di->opcode == I_MOVS) && (i == 0)) ||
            ((di->opcode == I_CMPS) && (i == 1))) {
          if (ci->dt != Decode64Bits) {
            ...
            isDefault = TRUE;
                   ...
                   ((di->opcode == I_MOVS) || (di->opcode == I_CMPS))) {
          if (ci->dt != Decode64Bits) {
            segment = R_DS;
            isDefault = FALSE;
          }
  ...
  */
  {
    ...
    /*
     * Always copy 16 bytes from the mnemonic, we have a sentinel padding so we
     * can read past. This helps the compiler to remove the call to memcpy and
     * therefore makes this copying much faster. The longest instruction is
     * exactly 16 chars long, so we null terminate the string below.
     */
...
#ifdef SUPPORT_64BIT_OFFSET
_DLLEXPORT_ _DecodeResult distorm_decode64(_OffsetType codeOffset,
                                           ...
                                           unsigned int* usedInstructionsCount)
#else
_DLLEXPORT_ _DecodeResult distorm_decode32(_OffsetType codeOffset,
                                           const unsigned char* code,
                                           int codeLen, _DecodeType dt,
                                           _DecodedInst result[],
                                           unsigned int maxInstructions,
...
#endif
{
  ...
  *usedInstructionsCount = 0;
  ...
  /* I use codeLen as a signed variable in order to ease detection of
   * underflow... and besides - */
  if (codeLen < 0) {
    return DECRES_INPUTERR;
  }
  ...
  /*
   * We have to format the result into text. But the interal decoder works with
   * the new structure of _DInst. Therefore, we will pass the result array(!)
   * from the caller and the interal decoder will fill it in with _DInst's. Then
   * we will copy each result to a temporary structure, and use it to reformat
   * that specific result.
   *
   * This is all done to save memory allocation and to work on the same result
   * array in-place!!! It's a bit ugly, I have to admit, but worth it.
   */
  ...
  for (i = 0; i < instsCount; i++) {
    /* distorm_format is optimized and can work with same input/output buffer
     * in-place. */
#ifdef SUPPORT_64BIT_OFFSET
...
_DLLEXPORT_ unsigned int distorm_version(void) { return __DISTORMV__; }
