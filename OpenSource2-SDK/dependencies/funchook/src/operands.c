...
uint32_t _REGISTERTORCLASS[] = /* Based on _RegisterType enumeration! */
    {RM_AX,  RM_CX,  RM_DX,  RM_BX,  RM_SP,  RM_BP,  RM_SI,  RM_DI,  RM_R8,
     RM_R9,  RM_R10, RM_R11, RM_R12, RM_R13, RM_R14, RM_R15, RM_AX,  RM_CX,
     RM_DX,  RM_BX,  RM_SP,  RM_BP,  RM_SI,  RM_DI,  RM_R8,  RM_R9,  RM_R10,
     RM_R11, RM_R12, RM_R13, RM_R14, RM_R15, RM_AX,  RM_CX,  RM_DX,  RM_BX,
     RM_SP,  RM_BP,  RM_SI,  RM_DI,  RM_R8,  RM_R9,  RM_R10, RM_R11, RM_R12,
     RM_R13, RM_R14, RM_R15, RM_AX,  RM_CX,  RM_DX,  RM_BX,  RM_AX,  RM_CX,
     RM_DX,  RM_BX,  RM_R8,  RM_R9,  RM_R10, RM_R11, RM_R12, RM_R13, RM_R14,
     RM_R15, RM_SP,  RM_BP,  RM_SI,  RM_DI,  RM_SEG, RM_SEG, RM_SEG, RM_SEG,
     RM_SEG, RM_SEG, 0,      RM_FPU, RM_FPU, RM_FPU, RM_FPU, RM_FPU, RM_FPU,
     RM_FPU, RM_FPU, RM_MMX, RM_MMX, RM_MMX, RM_MMX, RM_MMX, RM_MMX, RM_MMX,
     RM_MMX, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE,
     RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_AVX,
     RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX,
     RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_CR,  0,      RM_CR,
     RM_CR,  RM_CR,  0,      0,      0,      RM_CR,  RM_DR,  RM_DR,  RM_DR,
     RM_DR,  0,      0,      RM_DR,  RM_DR};

/* A helper function to fix the 8 bits register if REX is used (to support SIL,
 * DIL, etc). */
...
_INLINE_ void operands_set_tsi(_DInst* di, _Operand* op, _OperandType type,
                               uint16_t size, unsigned int index) {
  op->type = type;
 ...
 *	if mod == 11 <-- EXCEPTION, this is a general-purpose register and
 *mustn't lead to SIB decoding! ; So far so good, now the confusing part comes
 *in with mod == 0 and base=5, but no worry. if (mod == 00) { decode SIB byte
 *WITHOUT any displacement. EXCEPTION!!! when base == 5, read a 32 bits
 *displacement, but this time DO NOT use (EBP) BASE at all!
 *	}
 *
 *	NOTE: base could specify None (no base register) if base==5 and mod==0,
 *but then you also need DISP32.
 * }
 */
static void operands_extract_sib(_DInst* di, _PrefixState* ps,
                                 _DecodeType effAdrSz, unsigned int sib,
                                 unsigned int mod, _Operand* op) {
  ...
  /*
   * The following fields: base/index/scale/disp8/32 are ALL optional by
   * specific rules! The idea here is to keep the indirection as a simple-memory
   * type. Because the base is optional, and we might be left with only one
   * index. So even if there's a base but no index, or vice versa, we end up
   * with one index register.
   */
 ...
 * If you take a look carefully at Table 2-2. 32-Bit Addressing Forms with the
 ModR/M Byte,
 * you will understand it's easy to decode the operands.

 * First we check the DT, so we can decide according to which Table in the
 documentation we are supposed to decode.
 * Then we follow the specific table whether it's 16 bits or 32/64 bits.

 * Don't forget that Operand Size AND Address Size prefixes may change the
 decoding!
  ...
  if (effAdrSz != Decode16Bits) { /* Decode32Bits or Decode64Bits! */
    ...
    if ((rm == 5) && (mod == 0)) {
      ...
      if (ci->dt == Decode64Bits) {
        /* In 64 bits decoding mode depsite of the address size, a RIP-relative
         * address it is. */
        op->type = O_SMEM;
        op->index = R_RIP;
      ...
      if (mod == 1) {
        ...
        if (!read_stream_safe_sint8(ci, (int64_t*)&di->disp)) return FALSE;
                 ...
                 ((sib & 7) == 5)) { /* If there is no BASE, read DISP32! */
        di->dispSize = 32;
        if (!read_stream_safe_sint32(ci, (int64_t*)&di->disp)) return FALSE;
      }
    ...
    if ((mod == 0) && (rm == 6)) {
      ...
      /*
       * Create the O_MEM for 16 bits indirection that requires 2 registers,
       * E.G: [BS+SI]. or create O_SMEM for a single register indirection, E.G:
       * [BP].
       */
...
/*
 * This function is reponsible to textually format a required operand according
 to its type.
 * It is vital to understand that there are other operands than what the ModR/M
 byte specifies.

 * Only by decoding the operands of an instruction which got a LOCK prefix, we
 could tell whether it may use the LOCK prefix.
 * According to Intel, LOCK prefix must precede some specific instructions AND
 in their memory destination operand form (which means first operand).
 * LOCK INC EAX, would generate an exception, but LOCK INC [EAX] is alright.
 * Also LOCK ADD BX, [BP] would generate an exception.

 * Return code:
 * TRUE - continue parsing the instruction and its operands, everything went
 right 'till now.
 * FALSE - not enough bytes, or invalid operands.
 */

int operands_extract(_CodeInfo* ci, _DInst* di, _InstInfo* ii,
                     _iflags instFlags, _OpType type, unsigned int modrm,
                     _PrefixState* ps, _DecodeType effOpSz,
   ...
   * |------------------|
   */
  /* mod = (modrm >> 6) & 3; */ /* Mode(register-indirection,
                                   disp8+reg+indirection,
                                   disp16+reg+indirection, general-purpose
                                   register) */
  /* reg = (modrm >> 3) & 7; */ /* Register(could be part of the opcode itself
                                   or general-purpose register) */
  /* rm = modrm & 7; */ /* Specifies which general-purpose register or disp+reg
                           to use. */

  /* -- Memory Indirection Operands (that cannot be a general purpose register)
   * -- */
  if ((type >= OT_MEM) && (type <= OT_LMEM128_256)) {
    /* All of the above types can't use a general-purpose register (a MOD of
     * 3)!. */
    mod = (modrm >> 6) & 3;

    if (mod == 3) {
      if (type == OT_MEM_OPT) {
        /* Since the MEM is optional, only when mod != 3, then return true as if
         * the operand was alright. */
        return TRUE;
      }
      return FALSE;
    }
    ...
    switch (type) {
      ...
      case OT_MEM128:
        size = 128;
        break;
      case OT_MEM16_FULL: /* The size indicates about the second item of the
                             pair. */
        switch (effOpSz) {
          case Decode16Bits:
          ...
          case Decode64Bits:
            ...
            if ((instFlags & (INST_64BITS | INST_PRE_REX)) ==
                (INST_64BITS | INST_PRE_REX))
              ps->usedPrefixes |= INST_PRE_REX;
            size = 64;
            break;
        }
        break;
      case OT_MEM16_3264: /* The size indicates about the second item of the
                             pair. */
        if (ci->dt == Decode64Bits)
          size = 64;
        else
      ...
      case OT_MEM:
        size = 0; /* Size is unknown, but still handled. */
        break;
      default:
        return FALSE;
    }
  ...
  if ((type >= OT_RM8) && (type <= OT_LXMM64_128)) {
    ...
    if (mod != 3) {
      ...
      rm = modrm & 7;
      ret =
          operands_extract_modrm(ci, ps, di, effAdrSz, mod, rm, instFlags, op);
      op->size = (uint16_t)size;
      if ((op->type == O_SMEM) || (op->type == O_MEM)) {
        di->usedRegistersMask |= _REGISTERTORCLASS[op->index];
      }
      ...
      /*
       * General-purpose register is handled the same way in 16/32/64 bits
       * decoding modes. NOTE!! that we have to override the size of the
       * register, since it was set earlier as Memory and not Register!
       */
      rm = modrm & 7;
      size = 0;
      switch (type) {
        ...
        case OT_RM_FULL:
          switch (effOpSz) {
            ...
            case Decode64Bits:
              ...
              /* CALL NEAR/PUSH/POP defaults to 64 bits. --> INST_64BITS, REX
               * isn't required, thus ignored anyways. */
        ...
        case OT_R32_64_M16:
          /* FALL THROUGH, decode 32 or 64 bits register. */
        case OT_RM32_64: /* Take care specifically in MOVNTI/MOVD/CVT's
                            instructions, making it _REG64 with REX or if they
                            are promoted. */
          if (ps->vrex & PREFIX_EX_B) {
            ps->usedPrefixes |= INST_PRE_REX;
            rm += EX_GPR_BASE;
          }
          /* Is it a promoted instruction? (only INST_64BITS is set and REX
           * isn't required.) */
          if ((ci->dt == Decode64Bits) &&
              ((instFlags & (INST_64BITS | INST_PRE_REX)) == INST_64BITS)) {
            size = 64;
            rm += REGS64_BASE;
            break;
          }
          /* Give a chance to REX.W. Because if it was a promoted instruction we
           * don't care about REX.W anyways. */
          if (ps->vrex & PREFIX_EX_W) {
            ps->usedPrefixes |= INST_PRE_REX;
            size = 64;
            rm += REGS64_BASE;
          ...
          break;
        case OT_RM16_32: /* Used only with MOVZXD instruction to support 16 bits
                            operand. */
          if (ps->vrex & PREFIX_EX_B) {
            ps->usedPrefixes |= INST_PRE_REX;
            rm += EX_GPR_BASE;
        ...
        case OT_RM8:
          if (ps->prefixExtType == PET_REX) {
            ps->usedPrefixes |= INST_PRE_REX;
            rm = operands_fix_8bit_rex_base(
                rm + ((ps->vrex & PREFIX_EX_B) ? EX_GPR_BASE : 0));
          } else
            rm += REGS8_BASE;
          size = 8;
          break;
  ...
  switch (type) {
    ...
    case OT_IMM_FULL: /* 16, 32 or 64, depends on prefixes. */
      if (effOpSz == Decode16Bits) {
        ...
        case OT_IMM16: /* Force 16 bits imm. */
           ...
           * REX must be present because op size function takes it into
           * consideration.
           */
      } else if ((effOpSz == Decode64Bits) &&
                 ((instFlags & (INST_64BITS | INST_PRE_REX)) ==
                  (INST_64BITS | INST_PRE_REX))) {
        ps->usedPrefixes |= INST_PRE_REX;
    ...
    case OT_IMM32:
      ...
      if (ci->dt == Decode64Bits) {
         ...
         * Originally the op size was 64, but later was changed to reflect real
         * size of imm.
         */
        op->size = 32;
    ...
    case OT_SEIMM8: /* Sign extended immediate. */
       ...
       * Check the instFlags (ii->flags) if it can be operand-size-prefixed and
       * if the prefix exists.
       */
      op->type = O_IMM;
      if ((instFlags & INST_PRE_OP_SIZE) &&
    ...
    case OT_REG8:
      ...
      if (ps->prefixExtType) {
         ...
         * This is a PASSIVE behavior changer of REX prefix, it affects operands
         * even if its value is 0x40 !
         */
        ps->usedPrefixes |= INST_PRE_REX;
        op->index = (uint8_t)operands_fix_8bit_rex_base(
            reg + ((ps->vrex & PREFIX_EX_R) ? EX_GPR_BASE : 0));
      } else
    ...
    case OT_REG_FULL:
      switch (effOpSz) {
        ...
        case Decode64Bits: /* rex must be presented. */
          ...
          operands_set_tsi(
              di, op, O_REG, 64,
              REGS64_BASE + reg + ((ps->vrex & PREFIX_EX_R) ? EX_GPR_BASE : 0));
          break;
      }
      break;
    case OT_REG32:
      ...
      operands_set_tsi(di, op, O_REG, 32, REGS32_BASE + reg);
      break;
    case OT_REG32_64: /* Handle CVT's, MOVxX and MOVNTI instructions which could
                         be extended to 64 bits registers with REX. */
      if (ps->vrex & PREFIX_EX_R) {
        ps->usedPrefixes |= INST_PRE_REX;
        reg += EX_GPR_BASE;
      }

      /* Is it a promoted instruction? (only INST_64BITS is set and REX isn't
       * required.) */
      if ((ci->dt == Decode64Bits) &&
          ((instFlags & (INST_64BITS | INST_PRE_REX)) == INST_64BITS)) {
      ...
      if (ps->vrex & PREFIX_EX_W) {
        ...
        operands_set_tsi(di, op, O_REG, 32, REGS32_BASE + reg);
      break;
    case OT_FREG32_64_RM: /* Force decoding mode. Used for MOV CR(n)/DR(n) which
                             defaults to 64 bits operand size in 64 bits. */
      rm = modrm & 7;
      if (ps->vrex & PREFIX_EX_B) {
        ps->usedPrefixes |= INST_PRE_REX;
        rm += EX_GPR_BASE;
      ...
      if (ci->dt == Decode64Bits)
        operands_set_tsi(di, op, O_REG, 64, REGS64_BASE + rm);
      else
        operands_set_tsi(di, op, O_REG, 32, REGS32_BASE + rm);
      break;
    ...
    case OT_CREG:
       ...
       * Most of the CR's are not implemented, so if there's no matching string,
       * the operand is invalid.
       */
      if (ps->vrex & PREFIX_EX_R) {
        ps->usedPrefixes |= INST_PRE_REX;
        reg += EX_GPR_BASE;
      } else if ((ci->dt == Decode32Bits) &&
                 (ps->decodedPrefixes & INST_PRE_LOCK)) {
        /*
         * NOTE: In 32 bits decoding mode,
         * if the lock prefix is set before MOV CR(n) it will become the 4th bit
         * of the REG field like REX.R in 64 bits.
         */
        reg += EX_GPR_BASE;
        ps->usedPrefixes |= INST_PRE_LOCK;
      }
    ...
    case OT_SREG: /* Works with REG16 only! */
      if ((&di->ops[0] == op) && (reg == 1))
        return FALSE; /* Can't MOV CS, <REG>. */
      /*Don't parse if the reg exceeds the bounds of the array. */
      if (reg <= SEG_REGS_MAX - 1)
        operands_set_tsi(di, op, O_REG, 16, SREGS_BASE + reg);
      else
        return FALSE;
      break;
    case OT_SEG:
      op->type = O_REG;
      /* Size of reg is always 16, it's up to caller to zero extend it to
       * operand size. */
      op->size = 16;
      ps->usedPrefixes |= INST_PRE_OP_SIZE;
      /*
    ...
    case OT_ACC_FULL:
      if (effOpSz == Decode16Bits) {
        ...
        /* Only non-promoted instructions need REX in order to decode in 64
         * bits. */
      ...
      break;
    case OT_PTR16_FULL:
      /* ptr16:full - full is size of operand size to read, therefore Operand
       * Size Prefix affects this. So we need to handle it. */
      if (effOpSz == Decode16Bits) {
        ps->usedPrefixes |= INST_PRE_OP_SIZE;
        ...
        di->imm.ptr.seg =
            RUSHORT((ci->code + sizeof(int16_t))); /* And read segment. */
        ...
        di->imm.ptr.seg = RUSHORT(
            (ci->code + sizeof(int32_t))); /* And read segment, 16 bits. */
    ...
    case OT_MOFFS_FULL:
      ...
      /*
       * Just a pointer to a BYTE, WORD, DWORD, QWORD. Works only with
       * ACC8/16/32/64 respectively. MOV [0x1234], AL ; MOV AX, [0x1234] ; MOV
       * EAX, [0x1234], note that R/E/AX will be chosen by OT_ACC_FULL.
       */
    ...
    case OT_FPU_SSI:
      ...
      operands_set_tsi(di, op + 1, O_REG, 32,
                       FPUREGS_BASE + (*(ci->code - 1) & 7));
      di->opsNo++;
      break;
    case OT_FPU_SIS:
      operands_set_tsi(di, op, O_REG, 32, FPUREGS_BASE + (*(ci->code - 1) & 7));
     ...
     * INC/DEC (only 16/32 bits) /PUSH/POP/XCHG instructions, which get their
     REG from their own binary code.

     * Notice these instructions are 1 or 2 byte long,
     * code points after the byte which represents the instruction itself,
     * thus, even if the instructions are 2 bytes long it will read its last
     byte which contains the REG info.
     */
    case OT_IB_RB:
      /* Low 3 bits specify the REG, similar to the MODR/M byte reg. */
      operands_set_ts(op, O_REG, 8);
    ...
    case OT_IB_R_FULL:
      ...
      switch (effOpSz) {
        ...
        case Decode64Bits:
          /*
           * Automatically promoted instruction can drop REX prefix if not
           * required. PUSH/POP defaults to 64 bits. --> INST_64BITS MOV imm64 /
           * BSWAP requires REX.W to be 64 bits --> INST_64BITS | INST_PRE_REX
           */
     ...
     * If there's only the REP/NZ prefix, we won't output anything (All operands
     are implicit).
     * If there's an operand size prefix, we will change the suffix letter of
     the mnemonic, which specifies the size of operand to the required one.
     * If there's a segment override prefix, we will output the segment and the
     used index register (EDI/ESI).
     * If there's an address size prefix, we will output the (segment if needed
     and) the used and inverted index register (DI/SI).
     ...
     * The basic form of a repeatable instruction has its operands hidden and
     has a suffix letter
     * which implies on the size of operation being done.
     * Therefore, we cannot change the mnemonic here when we encounter another
     prefix and its not the decoder's responsibility to do so.
     * That's why the caller is responsible to add the suffix letter if no other
     prefixes are used.
    ...
    case OT_REGI_ESI:
      ...
      /* This might be a 16, 32 or 64 bits instruction, depends on the decoding
       * mode. */
    ...
    case OT_REGI_EDI:
      ...
      /* This might be a 16 or 32 bits instruction, depends on the decoding
       * mode. */
      if (instFlags & INST_16BITS) {
        ps->usedPrefixes |= INST_PRE_OP_SIZE;
      ...
      /* Note: The [rDI] operand can't be prefixed by a segment override,
       * therefore we don't set usedPrefixes. */
      if ((di->segment == R_NONE) && (ci->dt != Decode64Bits))
    ...
    case OT_REGI_EAX:
      /*
       * Implicit rAX as memory indirection operand. Used by AMD's SVM
       * instructions. Since this is a memory indirection, the default address
       * size in 64bits decoding mode is 64.
       */

      if (effAdrSz == Decode64Bits)
        operands_set_tsi(di, op, O_SMEM, 64, R_RAX);
      else if (effAdrSz == Decode32Bits) {
        ps->usedPrefixes |= INST_PRE_ADDR_SIZE;
