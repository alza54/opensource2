...
static _DecodeType decode_get_effective_addr_size(_DecodeType dt,
                                                  _iflags decodedPrefixes) {
  /*
...
static _DecodeType decode_get_effective_op_size(_DecodeType dt,
                                                _iflags decodedPrefixes,
                                                unsigned int rex,
                                                _iflags instFlags) {
  /*
   ...
   * Decode32 -> Decode16
   * Decode64 -> Decode16
   * Not that in 64bits it's a bit more complicated, because of REX and promoted
   * instructions.
   */
...
#define CONVERT_FLAGS_TO_EFLAGS(dst, src, field)            \
  dst->field = ((src->field & D_COMPACT_SAME_FLAGS) |       \
...
/* Important note: CI is keeping track only for code and codeLen, in case of a
 * failure caller has to restart on their own. */
static _DecodeResult decode_inst(_CodeInfo* ci, _PrefixState* ps,
                                 const uint8_t* startCode, _DInst* di) {
  /* Holds the info about the current found instruction. */
  _InstInfo* ii;
  _InstSharedInfo* isi;

  /* Calculate (and cache) effective-operand-size and effective-address-size
   * only once. */
  ...
  if (isPrefixed) {
    /*
     * If both REX and OpSize are available we will have to disable the OpSize,
     * because REX has precedence. However, only if REX.W is set! We had to wait
     * with this test, since the operand size may be a mandatory prefix, and we
     * know it only after fetching opcode.
     */
    if ((ps->decodedPrefixes & INST_PRE_OP_SIZE) &&
        (ps->prefixExtType == PET_REX) && (ps->vrex & PREFIX_EX_W) &&
    ...
    effOpSz = decode_get_effective_op_size(ci->dt, ps->decodedPrefixes,
                                           ps->vrex, instFlags);
  } else {
    effAdrSz =
        ci->dt; /* Default is current decoding type since there's no prefix. */
    effOpSz = decode_get_effective_op_size(ci->dt, 0, 0, instFlags);
  }

  /*
   * In this point we know the instruction we are about to decode and its
   operands (unless, it's an invalid one!),
   * so it makes it the right time for decoding-type suitability testing.
   * Which practically means, don't allow 32 bits instructions in 16 bits
   decoding mode, but do allow
   * 16 bits instructions in 32 bits decoding mode, of course...

   * NOTE: Make sure the instruction set for 32 bits has explicitly this
   specific flag set.
   * NOTE2: Make sure the instruction set for 64 bits has explicitly this
   specific flag set.

   * If this is the case, drop what we've got and restart all over after DB'ing
   that byte.

   * Though, don't drop an instruction which is also supported in 16 and 32
   bits.
   */

  /* ! ! ! DISABLED UNTIL FURTHER NOTICE ! ! ! Decode16Bits CAN NOW DECODE 32
   * BITS INSTRUCTIONS ! ! !*/
  /* if (ii && (dt == Decode16Bits) && (instFlags & INST_32BITS) && (~instFlags
   * & INST_16BITS)) ii = NULL; */

  memset(di, 0, sizeof(_DInst));

  if (instFlags & INST_MODRM_REQUIRED) {
    /* If the ModRM byte is not part of the opcode, skip the last byte code, so
     * code points now to ModRM. */
  ...
  ci->code++; /* Skip the last byte we just read (either last opcode's byte code
                 or a ModRM). */
   ...
   * This is necessary for the caller to know the size of rSP when using PUSHA
   * for example.
   */
  di->base = R_NONE;
  di->segment = R_NONE;
  ...
  if (isi->d != OT_NONE) {
    ...
    if (instFlags & (INST_MODRR_REQUIRED | INST_FORCE_REG0)) {
      ...
      if ((modrm < INST_DIVIDED_MODRM) && (instFlags & INST_MODRR_REQUIRED))
        goto _Undecodable;
      /* Some instructions enforce that reg=000, so validate that. (Specifically
       * EXTRQ). */
      if ((instFlags & INST_FORCE_REG0) && (((modrm >> 3) & 7) != 0))
        goto _Undecodable;
    }
    if (!operands_extract(ci, di, ii, instFlags, (_OpType)isi->d, modrm, ps,
                          effOpSz, effAdrSz, op++))
      goto _Undecodable;

    if (isi->s != OT_NONE) {
      if (!operands_extract(ci, di, ii, instFlags, (_OpType)isi->s, modrm, ps,
                            effOpSz, effAdrSz, op++))
        goto _Undecodable;
      opsNo++;
      /* Use third operand, only if the flags says this InstInfo requires it. */
      if (instFlags & INST_USE_OP3) {
        if (!operands_extract(ci, di, ii, instFlags,
                              (_OpType)((_InstInfoEx*)ii)->op3, modrm, ps,
                              effOpSz, effAdrSz, op++))
          goto _Undecodable;
        opsNo++;
        ...
        if (instFlags & INST_USE_OP4) {
          if (!operands_extract(ci, di, ii, instFlags,
                                (_OpType)((_InstInfoEx*)ii)->op4, modrm, ps,
                                effOpSz, effAdrSz, op++))
            goto _Undecodable;
          opsNo++;
        }
    ...
    /* Copy DST_WR flag. */
    di->flags |=
        (instFlags & INST_DST_WR) >>
        (31 -
         6); /* Copy bit from INST_DST_WR (bit 31) to FLAG_DST_WR (bit 6). */
    /* operands_extract may touched it for FPU operands, so add on top. */
    di->opsNo += (uint8_t)opsNo;
  }

  if (instFlags & (INST_3DNOW_FETCH | INST_PSEUDO_OPCODE | INST_NATIVE |
                   INST_PRE_REPNZ | INST_PRE_REP | INST_PRE_ADDR_SIZE |
                   INST_INVALID_64BITS | INST_64BITS_FETCH)) { /* 8 for 1! */

    /* If it's a native instruction copy OpSize Prefix. */
    if (ps && instFlags & INST_NATIVE)
      ps->usedPrefixes |= (ps->decodedPrefixes & INST_PRE_OP_SIZE);
    ...
    /* If it were a 3DNow! instruction, we will have to find the instruction
     * itself now that we got its operands extracted. */
    ...
    if (instFlags & INST_PSEUDO_OPCODE) {
      /* Used only for special CMP instructions which have pseudo opcodes
       * suffix. */
    ...
    if (instFlags & INST_PRE_ADDR_SIZE) {
      ...
      /* LOOPxx instructions are also native instruction, but they are special
         case ones, ADDR_SIZE prefix affects them. */
      else if (instFlags & INST_NATIVE) {
        di->opcode = ii->opcodeId;

        /* If LOOPxx gets here from 64bits, it must be Decode32Bits because
         * Address Size prefix is set. */
        ps->usedPrefixes |= INST_PRE_ADDR_SIZE;
      }
  ...
  /*
   * If we reached here the instruction was fully decoded, we located the
   * instruction in the DB and extracted operands. Use the correct mnemonic
   * according to the DT. If we are in 32 bits decoding mode it doesn't
   * necessarily mean we will choose mnemonic2, alas, it means that if there is
   * a mnemonic2, it will be used. Note: If the instruction is prefixed by
   * operand size we will format it in the non-default decoding mode! So there
   * might be a situation that an instruction of 32 bit gets formatted in 16
   * bits decoding mode. Both ways should end up with a correct and expected
  ...
  if (effOpSz == Decode32Bits) { /* Decode32Bits */
    ...
    if (instFlags & INST_USE_EXMNEMONIC) {
      ...
      ps->usedPrefixes |= INST_PRE_OP_SIZE;
             ...
             Decode64Bits) { /* Decode64Bits, note that some instructions might
                                be decoded in Decode32Bits above. */
    ...
    if (instFlags & (INST_USE_EXMNEMONIC | INST_USE_EXMNEMONIC2)) {
       ...
       * because they must not use REX (otherwise it will get to the wrong
       * instruction which share same opcode). See XRSTOR and XSAVEOPT.
       */
      if ((modrm >= INST_DIVIDED_MODRM) &&
          (instFlags & INST_MNEMONIC_MODRM_BASED))
        goto _Undecodable;
    ...
    /*
     * If it's a special instruction which has two mnemonics, then use the 16
     * bits one + update usedPrefixes. Note: use 16 bits mnemonic if that
     * instruction supports 32 bit or 64 bit explicitly.
     */
    if ((instFlags & (INST_USE_EXMNEMONIC | INST_32BITS | INST_64BITS)) ==
        INST_USE_EXMNEMONIC)
      ps->usedPrefixes |= INST_PRE_OP_SIZE;
  }
...
_SkipOpcoding:
  ...
  if (isPrefixed && (instFlags & INST_PRE_VEX) &&
      (((((_InstInfoEx*)ii)->flagsEx & INST_MNEMONIC_VEXW_BASED) &&
        (ps->vrex & PREFIX_EX_W)) ||
       ((((_InstInfoEx*)ii)->flagsEx & INST_MNEMONIC_VEXL_BASED) &&
        (ps->vrex & PREFIX_EX_L)))) {
    di->opcode = ((_InstInfoEx*)ii)->opcodeId2;
  }
   ...
   * start points to first prefix if any, otherwise it points to instruction
   * first byte.
   */
  ...
  if (ci->features & DF_FILL_EFLAGS) {
    ...
    if (isi->modifiedFlagsMask)
      CONVERT_FLAGS_TO_EFLAGS(di, isi, modifiedFlagsMask);
    if (isi->undefinedFlagsMask)
      CONVERT_FLAGS_TO_EFLAGS(di, isi, undefinedFlagsMask);
  }

  /*
   * Instruction can still be invalid if it's total length is over 15 bytes with
   * prefixes. Up to the caller to check that.
   */
  return DECRES_SUCCESS;

_Undecodable
    : /* If the instruction couldn't be decoded for some reason, fail. */
  /* Special case for WAIT instruction: If it's dropped as a prefix, we have to
   * return a valid instruction! */
  if (*startCode == INST_WAIT_INDEX) {
    ...
    /* Fix ci because WAIT could be a prefix that failed, and ci->code is now
     * out of sync. */
  ...
  /* Mark that we didn't manage to decode the instruction well, caller will drop
   * it. */
 ...
 * decode_internal
 *
 * supportOldIntr - Since now we work with new structure instead of the old
 * _DecodedInst, we are still interested in backward compatibility. So although,
 * the array is now of type _DInst, we want to read it in jumps of the old array
 * element's size. This is in order to save memory allocation for conversion
 * between the new and the old structures. It really means we can do the
 * conversion in-place now.
 */
_DecodeResult decode_internal(_CodeInfo* _ci, int supportOldIntr,
                              _DInst result[], unsigned int maxResultCount,
                              unsigned int* usedInstructionsCount) {
  _CodeInfo ci =
      *_ci; /* A working copy, we don't touch user's _ci except OUT params. */
  _PrefixState ps;
  ...
  _DInst* pdi = (_DInst*)&result[0]; /* There's always a room for at least one
                                        slot, checked earlier. */
  ...
  if (supportOldIntr) {
    ...
    maxResultAddr =
        (_DInst*)((size_t)&result[0] + (maxResultCount * sizeof(_DecodedInst)));
  } else
#endif /* DISTORM_LIGHT */
  {
   ...
   * Otherwise, we use the textual interface which needs full addresses for
   * formatting bytes output. So distorm_format will truncate later.
   */
  if (features & DF_MAXIMUM_ADDR32)
    ci.addrMask = 0xffffffff;
  else if (features & DF_MAXIMUM_ADDR16)
    ci.addrMask = 0xffff;
#endif

  ps.count = 1; /* Force zero'ing ps below. */

  /* Decode instructions as long as we have what to decode/enough room in
   * entries. */
  while (ci.codeLen > 0) {
    code = ci.code;
    codeLen = ci.codeLen;
    ...
    /* decode_inst keeps track (only if successful!) for code and codeLen but
     * ignores codeOffset, fix it here. */
    ci.codeOffset += pdi->size;

    if (ret == DECRES_SUCCESS) {
      if (features & (DF_SINGLE_BYTE_STEP | DF_RETURN_FC_ONLY |
                      DF_STOP_ON_PRIVILEGED | DF_STOP_ON_FLOW_CONTROL)) {
        /* Sync codeinfo, remember that currently it points to beginning of the
         * instruction and prefixes if any. */
        ...
        if ((features & DF_RETURN_FC_ONLY) &&
            (META_GET_FC(pdi->meta) == FC_NONE)) {
          continue;
        }

        /* Check whether we need to stop on any feature. */
        if ((features & DF_STOP_ON_PRIVILEGED) &&
            (FLAG_GET_PRIVILEGED(pdi->flags))) {
        ...
        if (features & DF_STOP_ON_FLOW_CONTROL) {
          ...
          if (mfc &&
              (((features & DF_STOP_ON_CALL) && (mfc == FC_CALL)) ||
  ...
  *usedInstructionsCount =
      (unsigned int)(((size_t)pdi - (size_t)result) / (size_t)diStructSize);
