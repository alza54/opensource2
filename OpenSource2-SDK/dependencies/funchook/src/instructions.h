 ...
 * Note "_FULL" suffix indicates to decode the operand as 16 bits or 32 bits
 * depends on DecodeType - actually, it depends on the decoding mode, unless
 * there's an operand/address size prefix. For example, the code: 33 c0 could be
 * decoded/executed as XOR AX, AX or XOR EAX, EAX.
 */
...
typedef enum OpType {
  ...
  /* Read a byte(8 bits) immediate and calculate it relatively to the current
     offset of the instruction being decoded */
  OT_RELCB,
  /* Read a word/dword immediate and calculate it relatively to the current
     offset of the instruction being decoded */
  OT_RELC_FULL,

  /*
   * Instruction-Block for one byte long instructions, used by
   * INC/DEC/PUSH/POP/XCHG, REG is extracted from the value of opcode Use a 8bit
   * register
   */
  OT_IB_RB,
  /* Use a 16/32/64bit register */
  ...
  /* Read an immediate as an absolute address, size is known by instruction,
     used by MOV (memory offset) only */
  OT_MOFFS8,
  OT_MOFFS_FULL,
  ...
  /* SSE registers: */
  OT_XMM,
  /* Extract the SSE register from the RM bits this time (used when the REG bits
     are used for opcode extension) */
  OT_XMM_RM,
  ...
  OT_YXMM,
  /* YMM/XMM (depends on prefix length) is encoded in the high nibble of an
     immediate byte. */
  OT_YXMM_IMM,
  ...
  /* Read one word (seg) and a word/dword/qword (depends on operand size),
     usually SEG:OFF, JMP 1234:1234 */
  OT_PTR16_FULL,
  ...
  /* MMX registers: */
  OT_MM,
  /* Extract the MMX register from the RM bits this time (used when the REG bits
     are used for opcode extension) */
  OT_MM_RM,
  ...
  OT_MEM128,
  /*
   * Read one word (seg), and a word/dword/qword (depends on operand size) from
   * memory. JMP FAR [EBX] means EBX point to 16:32 ptr.
   */
  OT_MEM16_FULL,
  /* Read one word (limit) and a dword/qword (limit) (depends on operand size),
     used by SGDT, SIDT, LGDT, LIDT. */
  OT_MEM16_3264,
  /* Used when a memory indirection is required, but if the mod field is 11,
     this operand will be ignored. */
  OT_MEM_OPT,

  /* Same as OT_RMXX but POINTS to 16 bits [cannot use GENERAL-PURPOSE REG!] */
  OT_FPUM16,
  /* Same as OT_RMXX but POINTS to 32 bits (single precision) [cannot use
     GENERAL-PURPOSE REG!] */
  OT_FPUM32,
  /* Same as OT_RMXX but POINTS to 64 bits (double precision) [cannot use
     GENERAL-PURPOSE REG!] */
  OT_FPUM64,
  /* Same as OT_RMXX but POINTS to 80 bits (extended precision) [cannot use
     GENERAL-PURPOSE REG!] */
  OT_FPUM80,
   ...
   * It supports all decoding modes, but if used as a memory indirection it's a
   * 16 bit ModR/M indirection.
   */
  OT_RFULL_M16,
  ...
  OT_WRM32_64,
  /*
   * Special type for SSE4, ModR/M might be a 32 bits or 64 bits (with REX)
   * register or a 8 bits memory indirection operand.
   */
  OT_R32_64_M8,
  /*
   * Special type for SSE4, ModR/M might be a 32 bits or 64 bits (with REX)
   * register or a 16 bits memory indirection operand.
   */
  OT_R32_64_M16,
  ...
  OT_R32_M8,
  /*
   * Special ModR/M for PINSRW, which need a 16 bits memory operand or 32 bits
   * register. In 16 bits decoding mode R32 becomes R16, operand size cannot
   * affect this.
   */
  OT_R32_M16,
...
/* Special treatment for instructions which are in the divided-category but
 * still needs the whole byte for ModR/M... */
#define INST_NOT_DIVIDED (1 << 1)
/*
 * Used explicitly in repeatable instructions,
 * which needs a suffix letter in their mnemonic to specify operation-size
 * (depend on operands).
 */
...
/*
 * Prefix flags (6 types: lock/rep, seg override, addr-size, oper-size, REX,
 * VEX) There are several specific instructions that can follow LOCK prefix,
 * note that they must be using a memory operand form, otherwise they generate
 * an exception.
 */
#define INST_PRE_LOCK (1 << 4)
/* REPNZ prefix for string instructions only - means an instruction can follow
 * it. */
...
/* Native instructions which needs suffix letter to indicate their
 * operation-size (and don't depend on operands). */
#define INST_NATIVE (1 << 15)
/* Use extended mnemonic, means it's an _InstInfoEx structure, which contains
 * another mnemonic for 32 bits specifically. */
#define INST_USE_EXMNEMONIC (1 << 16)
/* Use third operand, means it's an _InstInfoEx structure, which contains
 * another operand for special instructions. */
#define INST_USE_OP3 (1 << 17)
/* Use fourth operand, means it's an _InstInfoEx structure, which contains
 * another operand for special instructions. */
#define INST_USE_OP4 (1 << 18)
/* The instruction's mnemonic depends on the mod value of the ModR/M byte
 * (mod=11, mod!=11). */
#define INST_MNEMONIC_MODRM_BASED (1 << 19)
/* The instruction uses a ModR/M byte which the MOD must be 11 (for registers
 * operands only). */
#define INST_MODRR_REQUIRED (1 << 20)
/* The way of 3DNow! instructions are built, we have to handle their locating
 * specially. Suffix imm8 tells which instruction it is. */
#define INST_3DNOW_FETCH (1 << 21)
/* The instruction needs two suffixes, one for the comparison type (imm8) and
 * the second for its operation size indication (second mnemonic). */
#define INST_PSEUDO_OPCODE (1 << 22)
/* Invalid instruction at 64 bits decoding mode. */
#define INST_INVALID_64BITS (1 << 23)
/* Specific instruction can be promoted to 64 bits (without REX, it is promoted
 * automatically). */
#define INST_64BITS (1 << 24)
/* Indicates the instruction must be REX prefixed in order to use 64 bits
 * operands. */
...
/* Indicates that the instruction is encoded with a ModRM byte (REG field
 * specifically). */
#define INST_MODRM_INCLUDED (1 << 30)
/* Indicates that the first (/destination) operand of the instruction is
 * writable. */
...
#define INST_PRE_SEGOVRD_MASK32 \
  (INST_PRE_CS | INST_PRE_SS | INST_PRE_DS | INST_PRE_ES)
#define INST_PRE_SEGOVRD_MASK64 (INST_PRE_FS | INST_PRE_GS)
#define INST_PRE_SEGOVRD_MASK \
  (INST_PRE_SEGOVRD_MASK32 | INST_PRE_SEGOVRD_MASK64)
 ...
 * (MOD==11: got the right instruction, else skip +4 in prefixed table for the
 * correct instruction).
 */
#define INST_MODRR_BASED (1 << 5)
/* Indicates that the instruction doesn't use the VVVV field of the VEX prefix,
 * if it does then it's undecodable. */
#define INST_VEX_V_UNUSED (1 << 6)

/* Indication that the instruction is privileged (Ring 0), this should be
 * checked on the opcodeId field. */
...
/* CPU Flags that instructions modify, test or undefine, in compacted form
 * (CF,PF,AF,ZF,SF are 1:1 map to EFLAGS). */
...
#define D_COMPACT_SAME_FLAGS \
  (D_COMPACT_CF | D_COMPACT_PF | D_COMPACT_AF | D_COMPACT_ZF | D_COMPACT_SF)

/*
 * In order to save more space for storing the DB statically,
 * I came up with another level of shared info.
 * Because I saw that most of the information that instructions use repeats
 * itself.
 *
...
typedef struct {
  ...
  uint16_t meta; /* High byte = Instruction set class | Low byte = flow control
                    flags. */
} _InstSharedInfo;

/*
 * This structure is used for the instructions DB and NOT for the disassembled
 * result code! This is the BASE structure, there are extensions to this
 * structure below.
 */
typedef struct {
  uint16_t sharedIndex; /* An index into the SharedInfoTable. */
  uint16_t opcodeId; /* The opcodeId is really a byte-offset into the mnemonics
                        table. MSB is a privileged indication. */
} _InstInfo;

/*
 * There are merely few instructions which need a second mnemonic for 32 bits.
 * Or a third for 64 bits. Therefore sometimes the second mnemonic is empty but
 * not the third. In all decoding modes the first mnemonic is the default. A
 * flag will indicate it uses another mnemonic.
 *
 * There are a couple of (SSE4) instructions in the whole DB which need both op3
 * and 3rd mnemonic for 64bits, therefore, I decided to make the extended
 * structure contain all extra info in the same structure. There are a few
 * instructions (SHLD/SHRD/IMUL and SSE too) which use third operand (or a
 * fourth). A flag will indicate it uses a third/fourth operand.
 */
typedef struct {
  ...
  uint8_t flagsEx; /* 8 bits are enough, in the future we might make it a bigger
                      integer. */
...
/* Used to check instType < INT_INFOS, means we got an inst-info. Cause it has
