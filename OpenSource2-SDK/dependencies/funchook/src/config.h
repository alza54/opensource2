...
/*
 * If DISTORM_LIGHT is defined, everything involved in formatting the
 * instructions as text will be excluded from compilation. distorm_decode(..)
 * and distorm_format(..) will not be available. This will decrease the size of
 * the executable and leave you with decomposition functionality only.
 *
 * Note: it should be either set in the preprocessor definitions manually or in
 * command line -D switch. #define DISTORM_LIGHT
 */
 ...
 */
STATIC_INLINE int16_t RSHORT(const uint8_t *s) { return s[0] | (s[1] << 8); }
STATIC_INLINE uint16_t RUSHORT(const uint8_t *s) { return s[0] | (s[1] << 8); }
