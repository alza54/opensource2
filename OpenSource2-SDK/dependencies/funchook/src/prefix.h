...
/* Specifies the type of the extension prefix, such as: REX, 2 bytes VEX, 3
 * bytes VEX. */
typedef enum {
  PET_NONE = 0,
  PET_REX,
 ...
 * Other pointers are used to keep track of prefixes positions and help us know
 * if they appeared already and where.
 */
typedef struct {
  ...
  /* Indicates whether the operand size prefix (0x66) was used as a mandatory
   * prefix. */
...
/*
 * Intel supports 6 types of prefixes, whereas AMD supports 5 types (lock is
 * seperated from rep/nz). REX is the fifth prefix type, this time I'm based on
 * AMD64. VEX is the 6th, though it can't be repeated.
 */
...
_INLINE_ int prefixes_is_valid(unsigned char ch, _DecodeType dt) {
  /* The predicate selects (branchlessly) second half table for 64 bits
   * otherwise selects first half. */
...
_INLINE_ void prefixes_ignore(_PrefixState* ps, _PrefixIndexer pi) {
  /*
   * If that type of prefix appeared already, set the bit of that *former*
   * prefix. Anyway, set the new index of that prefix type to the current index,
   * so next time we know its position.
   */
