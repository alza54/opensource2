 ...
 * prefixed by a usable REX it will become XCHG, otherwise it will become a NOP.
...
/*
 * RDRAND and VMPTRLD share same 2.3 bytes opcode, and then alternates on the
 * MOD bits, RDRAND is OT_FULL_REG while VMPTRLD is OT_MEM, and there's no such
 * mixed type. So a hack into the inst_lookup was added for this decision, the
 * DB isn't flexible enough. :(
 */
