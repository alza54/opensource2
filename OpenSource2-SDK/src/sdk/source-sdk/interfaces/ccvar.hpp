...
namespace os2::sdk {
  namespace strings {
  inline const std::string log_ccvar_var_found =
      os2_string("os2::sdk::CCvar::FindVarByName() found '%s' at -> %p\n");
  inline const std::string log_ccvar_var_not_found =
      os2_string("os2::sdk::CCvar::FindVarByName() couldn't find '%s'.\n");
  };  // namespace strings
    ...
    auto FindVarByName(const char* var_name) -> ConVar* {
      ...
      while (i != 0xFFFFFFFF) {
        ...
        if (strcmp(pCvar->m_name, var_name) == 0) {
          LOG(strings::log_ccvar_var_found.c_str(), var_name, pCvar);
          return pCvar;
        }
        GetNextCvarIterator(i);
      }

      LOG(strings::log_ccvar_var_not_found.c_str(), var_name);
      return nullptr;
    }
  };
};  // namespace os2::sdk
